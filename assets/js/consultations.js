...
function saveConsultation() {
    const newConsultation = {
        ...
        doctor: "Dr. Smith",
        diagnosis:
            document.getElementById("diagnosis").value || "General Checkup",
        status: "Completed",
    };
...
function openNewConsultationModal() {
    ...
    DOM.get("consultationDateInput")?.addEventListener(
        "change",
        refreshTimeSlots,
    );
}
...
function setupConsultationPatientSearch() {
    ...
    searchInput.addEventListener("input", function () {
      ...
      `;
        ...
        resultsDiv.innerHTML = filteredPatients
            .map((p) => {
...
function saveAppointment() {
    ...
    if (duplicate) {
        showNotification(
            "This time slot is already booked with this doctor",
            "error",
        );
        return;
    }
    ...
    closeModal("newConsultationModal");
    showNotification(
        `Consultation scheduled for ${patient.name} on ${date} at ${time}`,
    );
