...
function loadRooms(roomsList = rooms) {
  ...
  DOM.setValue("totalRooms", roomsList.length);
  DOM.setValue("availableRooms", availableBedsCount);
  DOM.setValue("occupiedRooms", occupiedBedsCount);
  DOM.setValue("occupancyRate", `${occupancyRate}%`);
...
function viewRoomDetails(roomId) {
  ...
  const roomManagementModule = DOM.get('roomManagementModal');
...
`;
  ...
  DOM.setHTML("roomDetailsContent", html);
...
function filterRooms() {
  const wardFilter = DOM.getValue("wardFilter");
  const floorFilter = DOM.getValue("floorFilter");
  const statusFilter = DOM.getValue("statusFilter");
