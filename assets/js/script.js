...
document.addEventListener("DOMContentLoaded", () => {
  ...
  ------------------------- */
    function attachNavHandlers() {
        document
            .querySelectorAll(
                ".nav-item[data-page], .nav-submenu-item[data-page]",
            )
            .forEach((btn) => {
  ...
  ------------------------- */
    async function showPage(pageParam) {
        ...
        try {
            ...
            if (!res.ok)
                throw new Error(`Failed to load ${fetchHtml}: ${res.status}`);
        ...
        const initFnName =
            "initialize" + pageName.charAt(0).toUpperCase() + pageName.slice(1);
    ...
    function activateNavItem(pageName) {
        document
            .querySelectorAll(".nav-item, .nav-submenu-item")
            .forEach((el) => el.classList.remove("active"));
        document
            .querySelectorAll(
                `[data-page="${pageName}.html"], [data-page="${pageName}"]`,
            )
