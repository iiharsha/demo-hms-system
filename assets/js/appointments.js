...
function filterAppointmentsByDoctor(data, doctor) {
    ...
    const doctorName = doctor.includes("-")
        ? doctor.split("-")[1].trim()
        : doctor;
...
function openAddAppointmentModal() {
    ...
    DOM.get("appointmentDateInput")?.addEventListener(
        "change",
        refreshTimeSlots,
    );
}
...
function saveAppointment() {
    ...
    closeModal("addAppointmentModal");
    showNotification(
        `Appointment scheduled for ${patient.name} on ${date} at ${time}`,
    );
...
function onTimeSlotClick(event) {
    ...
    document
        .querySelectorAll(".appointment-slot.selected")
